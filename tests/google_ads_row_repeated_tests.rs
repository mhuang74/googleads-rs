...
use test_helpers::{AdBuilder, AdGroupAdBuilder, GoogleAdsRowBuilder};
...
fn test_responsive_search_ad_max_headlines() {
    ...
    let ad = AdBuilder::new()
        .with_responsive_search_ad(headlines.clone(), vec!["Description"], None, None)
        .build();
...
fn test_responsive_search_ad_long_descriptions() {
    ...
    let long_desc2 =
        "Another lengthy description with many words to test the handling of longer text";

    let ad = AdBuilder::new()
        .with_responsive_search_ad(vec!["Headline"], vec![long_desc1, long_desc2], None, None)
        .build();
