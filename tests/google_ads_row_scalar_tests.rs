...
use test_helpers::{
    AdGroupBuilder, CampaignBudgetBuilder, CampaignBuilder, CustomerBuilder, GoogleAdsRowBuilder,
...
#[test]
fn test_campaign_id() {
    let campaign = CampaignBuilder::new().id(123456789).build();
...
fn test_campaign_name() {
    let campaign = CampaignBuilder::new().name("Test Campaign 2024").build();
...
fn test_campaign_name_with_special_characters() {
    ...
    assert_eq!(
        ...
        "Test Campaign: Q4 2024 - \"Sale\""
    );
}

#[test]
fn test_campaign_end_date_time() {
    let campaign = CampaignBuilder::new().end_date_time("2024-12-31T23:59:59Z").build();

    let row = GoogleAdsRowBuilder::new().with_campaign(campaign).build();

    assert_eq!(row.get("campaign.end_date_time"), "2024-12-31T23:59:59Z");
}
...
fn test_campaign_optimization_score() {
    let campaign = CampaignBuilder::new().optimization_score(0.85).build();
...
#[test]
fn test_ad_group_id() {
    let ad_group = AdGroupBuilder::new().id(987654321).build();
...
fn test_ad_group_name() {
    let ad_group = AdGroupBuilder::new().name("Brand Keywords").build();
...
#[test]
fn test_customer_id() {
    let customer = CustomerBuilder::new().id(1234567890).build();
...
fn test_customer_descriptive_name() {
    let customer = CustomerBuilder::new().descriptive_name("Acme Corp").build();
...
fn test_customer_currency_code() {
    let customer = CustomerBuilder::new().currency_code("USD").build();
...
fn test_customer_time_zone() {
    let customer = CustomerBuilder::new().time_zone("America/New_York").build();
...
fn test_metrics_impressions() {
    let metrics = MetricsBuilder::new().impressions(10000).build();
...
fn test_metrics_clicks() {
    let metrics = MetricsBuilder::new().clicks(500).build();
...
#[test]
fn test_metrics_ctr() {
    let metrics = MetricsBuilder::new().ctr(0.05).build();
...
fn test_metrics_conversions() {
    let metrics = MetricsBuilder::new().conversions(25.5).build();
...
fn test_metrics_conversions_value() {
    let metrics = MetricsBuilder::new().conversions_value(1234.56).build();
...
fn test_metrics_average_cpc() {
    let metrics = MetricsBuilder::new().average_cpc(3.45).build();
...
fn test_metrics_average_cpm() {
    let metrics = MetricsBuilder::new().average_cpm(12.75).build();
...
fn test_segments_hour() {
    let segments = SegmentsBuilder::new().hour(14).build();
...
fn test_segments_month() {
    let segments = SegmentsBuilder::new().month("2024-10").build();
...
fn test_segments_year() {
    let segments = SegmentsBuilder::new().year(2024).build();
...
fn test_campaign_labels_empty() {
    let campaign = CampaignBuilder::new().labels(vec![]).build();
...
fn test_ad_group_labels_multiple() {
    ...
    assert_eq!(
        row.get("ad_group.labels"),
        "customers/123/labels/1, customers/123/labels/2"
    );
}
...
#[test]
fn test_zero_values() {
    let campaign = CampaignBuilder::new().id(0).build();
...
fn test_negative_values() {
    let ad_group = AdGroupBuilder::new().cpc_bid_micros(-100).build();
...
fn test_empty_string_values() {
    let campaign = CampaignBuilder::new().name("").build();
...
fn test_floating_point_precision() {
    let metrics = MetricsBuilder::new().ctr(0.12345678901234567890).build();
...
fn test_multiple_resources_in_row() {
    let campaign = CampaignBuilder::new().id(111).name("Campaign A").build();

    let ad_group = AdGroupBuilder::new().id(222).name("AdGroup B").build();

    let metrics = MetricsBuilder::new().impressions(1000).clicks(50).build();
